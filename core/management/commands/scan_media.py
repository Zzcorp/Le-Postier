...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if animated_dir.exists():
            ...
            self.stdout.write(f'  Created animated_cp directory')

        # Create database entries if requested OR if database is empty
        total_postcards = Postcard.objects.count()

        if total_postcards == 0 and found_numbers:
            self.stdout.write(self.style.WARNING(
                f'\n⚠️  Database is empty but found {len(found_numbers)} images!'
            ))
            self.stdout.write(self.style.WARNING('   Auto-creating database entries...'))
            create_entries = True
        ...
        if create_entries and found_numbers:
            ...
            for number in sorted(found_numbers):
                ...
                # Check if has_animation field exists and update it
                ...
                if needs_update and not was_created:
                    try:
                        ...
                        self.stdout.write(self.style.WARNING(f'Error updating {number}: {e}'))
        ...
        try:
            ...
            Postcard._meta.get_field('has_animation')
            with_animation = Postcard.objects.filter(has_animation=True).count()
        except Exception as e:
            self.stdout.write(self.style.WARNING(f'  Note: has_animation field issue: {e}'))
